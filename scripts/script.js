...
let timer = setInterval(() => {
  ...
  let distanceMinutes = Math.floor(
    (distanceDate % (1000 * 60 * 60)) / (1000 * 60)
  );
  let distanceSeconds = Math.floor((distanceDate % (1000 * 60)) / 1000);

  daysElem.innerHTML = distanceDays < 10 ? "0" + distanceDays : distanceDays;
  hoursElem.innerHTML =
    distanceHours < 10 ? "0" + distanceHours : distanceHours;
  minutesElem.innerHTML =
    distanceMinutes < 10 ? "0" + distanceMinutes : distanceMinutes;
  secondsElem.innerHTML =
    distanceSeconds < 10 ? "0" + distanceSeconds : distanceSeconds;
